...
const { postToWebhook } = require('../utils/webhookNotifier');
  ...
  try {
    ...
    for (const user of subscribedUsers) {
      ...
      const dueInvoices = await Invoice.find({
        ...
        status: { $in: ['unpaid', 'overdue'] }, // Or whichever statuses indicate the invoice is due
      });
        ...
        const payload = {
          ...
          dueInvoices: dueInvoices.map(invoice => ({
            ...
            dueDate: invoice.dueDate
              ? new Date(invoice.dueDate).toLocaleDateString('en-GB') // Format as DD/MM/YYYY
              : null,
...
// This example runs every Monday at 9 AM (server time):
cron.schedule('0 9 * * 1', () => {
